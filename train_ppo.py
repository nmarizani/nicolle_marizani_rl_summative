...
def train_ppo(
    ...
    env = Monitor(env, filename=f"logs/ppo/{save_name}")
    ...
    model = PPO(
        ...
        verbose=1,
        tensorboard_log=f"logs/ppo/{save_name}_tb"
    )
    
    # Callbacks
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=f"saved_models/ppo/{save_name}_best",
        log_path=f"logs/ppo/{save_name}_eval",
        eval_freq=5000,
        deterministic=True,
        render=False
    )
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=10000,
        save_path=f"saved_models/ppo/{save_name}_checkpoints",
    ...
    # Save final model
    model.save(f"saved_models/ppo/{save_name}")
    print(f"\n✓ Model saved to saved_models/ppo/{save_name}.zip")
...
def hyperparameter_tuning():
    ...
    print("\n" + "="*60)
    print("HYPERPARAMETER TUNING - PPO")
    ...
    configs = [
        ...
        # Config 2: Lower LR
        {"learning_rate": 0.0001, "n_steps": 2048, "batch_size": 64, "n_epochs": 10, "gamma": 0.99},
        # Config 3: Higher LR
        {"learning_rate": 0.0005, "n_steps": 2048, "batch_size": 64, "n_epochs": 10, "gamma": 0.99},
        # Config 4: More steps
        {"learning_rate": 0.0003, "n_steps": 4096, "batch_size": 64, "n_epochs": 10, "gamma": 0.99},
        # Config 5: Larger batch
        {"learning_rate": 0.0003, "n_steps": 2048, "batch_size": 128, "n_epochs": 10, "gamma": 0.99},
        # Config 6: More epochs
        {"learning_rate": 0.0003, "n_steps": 2048, "batch_size": 64, "n_epochs": 20, "gamma": 0.99},
        # Config 7: Different gamma
        {"learning_rate": 0.0003, "n_steps": 2048, "batch_size": 64, "n_epochs": 10, "gamma": 0.95},
        # Config 8: Higher gamma
        {"learning_rate": 0.0003, "n_steps": 2048, "batch_size": 64, "n_epochs": 10, "gamma": 0.995},
        # Config 9: Combined adjustments
        {"learning_rate": 0.0001, "n_steps": 4096, "batch_size": 128, "n_epochs": 15, "gamma": 0.995},
        # Config 10: Aggressive
        {"learning_rate": 0.0005, "n_steps": 1024, "batch_size": 32, "n_epochs": 5, "gamma": 0.95},
    ]
    ...
    for i, config in enumerate(configs):
        ...
        model = train_ppo(
            ...
            gamma=config['gamma'],
            save_name=f"ppo_tune_{i+1}"
        )
        
        rewards, detections, severe = evaluate_model(
            f"saved_models/ppo/ppo_tune_{i+1}.zip",
            num_episodes=5
        )
    ...
    # Save results
    with open("saved_models/ppo/tuning_results.txt", "w") as f:
        f.write("PPO Hyperparameter Tuning Results\n")
        f.write("="*60 + "\n\n")
    ...
    print("\n✓ Results saved to saved_models/ppo/tuning_results.txt")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "tune":
        ...
        print("\nTraining PPO with default hyperparameters...")
        print("For hyperparameter tuning, run: python train_ppo.py tune\n")
        ...
        evaluate_model("saved_models/ppo/ppo_model.zip", num_episodes=10)
        
        print("\n✓ Training complete!")
        print("To visualize: python visualize.py saved_models/ppo/ppo_model.zip")
